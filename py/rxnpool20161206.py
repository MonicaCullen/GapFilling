                ...
                for c,c_origrs in enumerate(origRxnRsSmi):
                    ...
                    if c_origrs.count('$') == 0:
                        for d,d_sub in enumerate(patRsSmi):
                            ...
                            if not has :
                                c_origrs_mol = mfsmi(c_origrs)  
                                if not bool(c_origrs_mol):  
                                    c_origrs_mol = mfsma(c_origrs)  
    ...
    if bool(prePsSmi_preRsSmiRxnid):
        ...
        for ps,rsinfo in prePsSmi_preRsSmiRxnid.items():
            ...
            for rs,idlist in rsinfo.items():
                ...
                for _id in idlist:
                    ref+=1
                    preResultsDic[preRxnNum]['ref %s' % ref] = dict()  
                    preResultsDic[preRxnNum]['ref %s' % ref]['rxnId'] = _id  
                    preResultsDic[preRxnNum]['ref %s' % ref]['ecNum'] = rhea_ECAssigner[_id]["ecnumber"]  
                    preResultsDic[preRxnNum]['ref %s' % ref]['maxSi'] = max(ridSubSimilarity[_id].values())  
                    preResultsDic[preRxnNum]['ref %s' % ref]['subSi'] = ridSubSimilarity[_id]  
                ...
                rangeIndex[preRxnNum] = max([preResultsDic[preRxnNum][r]['maxSi']for r in preResultsDic[preRxnNum].keys() if r.count('ref')]) #用来排序  
        ...
        rangeIndex = sorted(rangeIndex.iteritems(),key = lambda t:t[1],reverse = True)  
        with open(os.path.join(resultDir,'range.json'),"w") as f3:  
            json.dump(rangeIndex,f3,indent = 2)  
...
def NeutraliseCharges(smiles, reactions=None):
    ...
    if not mol:
        ...
        if replaced:
            return (mtsmi(mol), True) 
        else:
            return (mtsmi(mol), False) 
...
def standSmiNameId():
    ...
    ChEBISmiName = json.load(open('./ChEBISmi_NameId.json')) 
    StandSmiName = dict()
    ...
    for m,it in enumerate([_rs,_ps]):
        ...
        for s in it:
            ...
            try:
                ...
                sId = StandSmiName[stand_s]["ChEBI ID"][0] 
            except:
            ...
            if num != 1:
                ...
                index3[str(m)].append(str(num) + ' ' + str(sId)) 
...
def main():
    ...
    # queryRsSmi ="C1=CC2C(C1)C1CC2C=C1"
    # queryPsSmi ="C1CC2C(C1)C1CC2CC1"

    # queryRsSmi ="c1ccccc1C=O"
    # queryPsSmi ="c1ccccc1C=O[OH]"
